...
describe('/artists', () => {
  ...
  describe('PATCH /artists/:artistId', () => {
    it('updates artist genre record by id', (done) => {
      const artist = artists[0];
      chai.request(server)
    ...
    it('updates artist name record by id', (done) => {
      const artist = artists[0];
      chai.request(server)
        .patch(`/artists/${artist._id}`)
        .send({ name: 'The Rolling Stones' })
        .end((err, res) => {
          expect(err).to.equal(null);
          expect(res.status).to.equal(200);
          Artist.findById(artist._id, (err, updatedArtist) => {
            expect(updatedArtist.name).to.equal('The Rolling Stones');
